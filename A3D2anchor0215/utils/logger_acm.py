...
class ResultRecorder(object):
    def __init__(self, path, total_cv=10):
        ...
        if not os.path.exists(self.path):
            f = open(self.path, 'w')
            f.write('acc\tuar\tf1\tweight_p\tweight_t\n')   
            f.close()
    ...
    def write_result_to_tsv(self, results, cvNo):
        ...
        # content[cvNo] = '{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{}\t{:.4f}\n'.format(results['acc'], results['uar'], results['f1'], results['weight'], results['BESTepoch'], results['f1_best'])
        content[cvNo] = '{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\n'.format(results['acc'], results['uar'], results['f1'], results['weight_p'], results['weight_t'])  # acm
