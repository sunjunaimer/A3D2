...
class SimpleFC(nn.Module):
    def __init__(self, input_dim, output_dim, hidden=[], dropout=0.3):
        super(SimpleFC, self).__init__()
        
        layers = []
        current_dim = input_dim

        # 循环 hidden 数组中的元素，逐层加入网络
...
class SimpleFC2(nn.Module):
    def __init__(self, input_dim, output_dim, hidden=[], d_f=16, dropout=0.3):
        ...
        # 添加输出层
        layers.append(nn.Linear(current_dim, d_f))
        layers.append(nn.BatchNorm1d(d_f))
        layers.append(nn.ReLU())
        layers.append(nn.Dropout(dropout))
        fl = []
        fl.append(nn.Linear(d_f, output_dim))
        fl.append(nn.BatchNorm1d(output_dim))
        fl.append(nn.ReLU())


        # 使用 nn.Sequential 包装所有层
        self.model = nn.Sequential(*layers)
        self.head = nn.Sequential(*fl)
...
class NwdClassifier(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(NwdClassifier, self).__init__()
        self.layer1 = nn.Linear(input_dim, 64)
        self.layer2 = nn.ReLU()
        self.layer3 = nn.Linear(64, output_dim)
        self.head = self.layer3  # 设置输出层为头部

    def forward(self, x):
        f = self.layer2(self.layer1(x))
        y = self.head(f)
        return y, f
