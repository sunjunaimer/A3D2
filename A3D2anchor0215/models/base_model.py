    ...
    def eval(self):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if isinstance(net, list):  # 处理 net 是列表的情况
                    for sub_net in net:
                        sub_net.eval()  # 设置每个子网络为 eval 模式
                else:  # 处理单个网络
                    net.eval()  # 直接设置 eval 模式
    ...
    def train(self):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if isinstance(net, list):  # 处理 net 是列表的情况
                    for sub_net in net:
                        sub_net.train()  # 设置每个子网络为 train 模式
                else:  # 处理单个网络
                    net.train()  # 直接设置 train 模式
    ...
    def save_networks(self, epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if isinstance(net, list):  # 如果 net 是列表
                    for idx, sub_net in enumerate(net):
                        save_filename = '%s_net_%s_%d.pth' % (epoch, name, idx)  # 给每个子网络加索引
    ...
    def load_networks(self, epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if isinstance(net, list):  # 处理 net 是列表的情况
                    for idx, sub_net in enumerate(net):
                        load_filename = '%s_net_%s_%d.pth' % (epoch, name, idx)  # 对每个子网络添加索引
                        load_path = os.path.join(self.save_dir, load_filename)

                        if isinstance(sub_net, torch.nn.DataParallel):  # 兼容 DataParallel
                            sub_net = sub_net.module

                        print('loading the model from %s' % load_path)
                        state_dict = torch.load(load_path, map_location=self.device)

                        if hasattr(state_dict, '_metadata'):
                            del state_dict._metadata  # 移除元数据，防止潜在问题

                        sub_net.load_state_dict(state_dict)  # 加载权重
                else:  # 处理单个网络
                    load_filename = '%s_net_%s.pth' % (epoch, name)
                    load_path = os.path.join(self.save_dir, load_filename)
                    ...
                    net.load_state_dict(state_dict)  # 加载权重
