...
def lora_apvit2(model, r):
    ...
    # Define specific r values for the last 3 layers
    r_values = {5: 8, 6: 16, 7: 32}

    # Define the target modules for LoRA injection
    target_modules = (
        [f"vit.blocks.{i}.attn.qkv" for i in range(layer_s, layer_e)]
        + [f"vit.blocks.{i}.attn.proj" for i in range(layer_s, layer_e)]
        + [f"vit.blocks.{i}.mlp.fc1" for i in range(layer_s, layer_e)]
        + [f"vit.blocks.{i}.mlp.fc2" for i in range(layer_s, layer_e)]
    )

    # Create and apply a custom LoraConfig for each layer
    for i in range(layer_s, layer_e):
        # Use specific r value if defined, otherwise use the default r value
        lora_config = LoraConfig(
            r=r_values.get(i, lora_r),
...
def lora_bert(model, r):
    ...
    # Define r values for the last 3 layers
    r_values = {9: 8, 10: 16, 11: 32}

    target_modules = (
        [f"encoder.layer.{i}.attention.self.{proj}" for i in range(layer_s, layer_e) for proj in ["query", "key", "value", "output.dense"]]
    ...
    # Create a custom LoraConfig for each layer
    custom_lora_configs = []
    for i in range(layer_s, layer_e):
        custom_lora_configs.append(
            LoraConfig(
                r=r_values.get(i, lora_r),
                lora_alpha=lora_alpha,
                lora_dropout=0.1,
                target_modules=[
                    ...
                    f"encoder.layer.{i}.attention.self.output.dense",
    ...
    for lora_config in custom_lora_configs:
        model = get_peft_model(model, lora_config)
...
def lora_wavlm(model, r):
    ...
    for i in range(layer_s, layer_e):
        ...
        lora_config = LoraConfig(
            ...
            bias="none",
            target_modules=[
                f"encoder.layers.{i}.attention.k_proj",
                f"encoder.layers.{i}.attention.v_proj",
                f"encoder.layers.{i}.attention.q_proj",
                f"encoder.layers.{i}.attention.out_proj",
                f"encoder.layers.{i}.feed_forward.intermediate_dense",
                f"encoder.layers.{i}.feed_forward.output_dense",
            ],
        )
        # Apply the configuration to the model
        model = get_peft_model(model, lora_config)
